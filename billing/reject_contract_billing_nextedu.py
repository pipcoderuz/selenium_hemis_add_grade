...
import pandas as pd
from config import BILLING_N_LOGIN_VALUE, BILLING_N_PASSWORD_VALUE
...
# ==================== LOGIN ====================
def login():
    """Billing tizimiga oddiy login/parol orqali kirish"""
    print("Billing login sahifasiga o'tyapman...")
    driver.get("https://billing.nextedu.uz/")
    time.sleep(2)
    ...
    try:
        ...
        username_input.send_keys(BILLING_N_LOGIN_VALUE)
        print("Username kiritildi")

        password_input = driver.find_element(By.ID, "password")
        password_input.clear()
        password_input.send_keys(BILLING_N_PASSWORD_VALUE)
    ...
    try:
        driver.get(
            "https://billing.nextedu.uz/financial-activity/contracts?academicYearId=3")
        time.sleep(2)
...
# ==================== SAHIFANI YANGILASH ====================
def sahifani_yangilash():
    """Sahifani to'liq yangilash va qidiruvni tozalash"""
    try:
        # Sahifani yangilash
        driver.get(
            "https://billing.nextedu.uz/financial-activity/contracts?academicYearId=3")
        ...
        print(f"  ⚠ Sahifani yangilashda xatolik: {e}")
        return False
...
def shartnoma_qayta_ishlash(contract_number, row_index, total_count):
    ...
    try:
        ...
        try:
            ...
            print(f"  ✓ Qidiruv: {contract_number}")
            time.sleep(2)
        except Exception as e:
        ...
        try:
            if len(driver.window_handles) > 1:
                driver.close()
                driver.switch_to.window(driver.window_handles[0])
                print(f"  ✓ Yangi oyna yopildi")
        except:
            pass

        # ==================== MUHIM: Sahifani yangilash ====================
        print(f"  🔄 Sahifa yangilanmoqda...")
        sahifani_yangilash()
...
def main():
    print("\n" + "="*60)
    print("🏛️ BILLING SHARTNOMALAR BOTI (nextedu)")
    print("="*60)
    ...
    for index, row in df.iterrows():
        ...
        if natija:
            ...
            # Muvaffaqiyatsiz bo'lsa ham sahifani yangilash
            sahifani_yangilash()
